...
namespace PanteonStrategyDemo.Concretes.GameBoardData
{
    public class PlacementContidions
    {
        public void UpdateTiles(float xPosition, float yPosition, int cellWidth, int cellHeight)
        {
            ...
            for (int i = 0; i < cellHeight; i++)
            {
                ...
                {
                    BoardManager.Instance.BoardTiles[i + row, j + column].IsValid = false;
                }
            }
        }

        public bool IsPlaceAvailable(float xPosition, float yPosition, int cellHeight, int cellWidth)
        {
            GetTileOverCursor(xPosition, yPosition, out int row, out int column);
            ...
            for (int i = 0; i < cellHeight; i++)
            {
                for (int j = 0; j < cellWidth; j++)
                {
                    if (!BoardManager.Instance.BoardTiles[i + row, j + column].IsValid)
                    {
                        return false;
                    }
                }
